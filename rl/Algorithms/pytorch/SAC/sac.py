...
class SAC:
    ...
    def update(self, batch, k_iter):
        """learn model"""
        batch_state = FLOAT(np.array(batch.state)).to(device)
        batch_action = FLOAT(np.array(batch.action)).to(device)
        batch_reward = FLOAT(np.array(batch.reward)).to(device)
        batch_next_state = FLOAT(np.array(batch.next_state)).to(device)
        batch_mask = FLOAT(np.array(batch.mask)).to(device)
