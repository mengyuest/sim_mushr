...
class GAIL:
    def __init__(self,
        ...
        # /datadrive/mushr_data/mushr_gail_trajs.npz
        if "AMLT_DATA_DIR" in os.environ and len(os.environ["AMLT_DATA_DIR"]) > 1:
            expert_data_path = os.environ["AMLT_DATA_DIR"] + "/mushr_data/" + expert_data_path.split("/")[-1]
        if "soft" not in expert_data_path and "hard" not in expert_data_path:
            if args.reward_type=="hard":
                expert_data_path = expert_data_path.replace(".npz","_hard.npz")
            else:
                expert_data_path = expert_data_path.replace(".npz","_soft.npz")
    ...
    def learn(self, writer, i_iter):
        ...
        gen_batch_state = FLOAT(np.array(gen_batch.state)).to(
            device)  # [batch size, state size]
        gen_batch_action = FLOAT(np.array(gen_batch.action)).to(
            device)  # [batch size, action size]
        gen_batch_old_log_prob = FLOAT(
            np.array(gen_batch.log_prob)).to(device)  # [batch size, 1]
        gen_batch_mask = FLOAT(np.array(gen_batch.mask)).to(device)  # [batch, 1]
        ...
        for _ in range(ppo_optim_i_iters):
            if ppo_mini_batch_size > 0:
                ...
                for i in range(optim_iter_num):
                    ...
                    d_tmp = ppo_step(policy_net=self.policy,
                    ...
                    v_loss, p_loss, ent_loss = d_tmp["critic_loss"], d_tmp["policy_loss"], d_tmp["policy_entropy"]
            else:
                d_tmp = ppo_step(policy_net=self.policy,
                                                    value_net=self.value,
                                                    optimizer_policy=self.optimizer_policy,
